...
func defaultKeyFunc(c *gin.Context) string {
	log.Printf("请求头:%v,%v,%v\n", c.Request.Method, c.Request.Host+c.Request.URL.RequestURI(), c.Request.Header.Get("Authorization"))
	//uid := ParseUUID(c)
	//if uid == "" {
	//	uid = c.Request.Header.Get("Authorization")
	//}
	//log.Printf(uid)
	raw := c.Request.Method + ":" + c.Request.Host + c.Request.URL.RequestURI()
